...
public:
    ...
    // 植物管理相关函数
    void removePlantFromGrid(int row, int col); // 从网格移除植物
    void handleCherryBombExplosion(int row, int col); // 处理樱桃炸弹爆炸伤害
private:
    // 游戏背景元素
    ...
    cocos2d::Vector<cocos2d::Sprite*>& getSunshines(); // 获取阳光容器的引用，供植物系统使用
    ...
    Plant* gridPlants[GRID_ROWS][GRID_COLS]; // 网格中的植物指针数组
    
    // 植物卡片冷却时间相关
    std::map<PlantType, float> cardCooldowns;   // 各植物类型的冷却时间
    std::map<PlantType, float> cardCooldownTimers; // 各植物类型的冷却时间计时器
    Vector<Sprite*> cardCooldownMasks;          // 冷却遮罩精灵容器
    std::map<PlantType, Sprite*> plantTypeToCardMap; // 植物类型到卡片的映射
    std::map<PlantType, Sprite*> plantTypeToCooldownMaskMap; // 植物类型到冷却遮罩的映射
    ...
    void initBulletSystem();      // 初始化子弹系统
    void updateBullets(float dt); // 更新子弹状态
    void checkBulletZombieCollision(); // 检查子弹和僵尸的碰撞
    void checkBulletFireTreeCollision(); // 检查子弹和火炬树桩的碰撞
    void removeDeadBullets();     // 移除无效的子弹
public:
    void addBullet(Bullet* bullet); // 添加子弹到容器
